...
const BANK_STYLES = {
  花旗銀行: ['#082f49', '#164e63', '#22d3ee'], 渣打銀行: ['#052e16', '#166534', '#4ade80'],
  恆生銀行: ['#451a03', '#92400e', '#fbbf24'], 滙豐銀行: ['#4c0519', '#9f1239', '#fb7185'],
  中銀香港: ['#450a0a', '#991b1b', '#f87171'], 建行亞洲: ['#1e1b4b', '#4338ca', '#a5b4fc'],
  其他銀行: ['#172033', '#334155', '#cbd5e1']
};
...
function Button({ children, variant = 'ghost', ...props }) { return <button className={`button button-${variant}`} {...props}>{children}</button>; }
...
function CardsView({ cards, open, edit, toggle, remove, updateDate }) { return <div className="view-stack"><div className="view-toolbar"><div><p className="eyebrow">YOUR WALLET</p><h2>信用卡管理 <span className="count">{cards.length}</span></h2></div><Button variant="primary" onClick={open}><Plus size={16} />新增信用卡</Button></div><div className="card-grid">{cards.map((card) => <CardTile key={card.id} card={card} edit={edit} toggle={toggle} remove={remove} updateDate={updateDate} />)}</div>{!cards.length && <Glass><Empty icon={CreditCard}>還沒有信用卡，新增第一張卡開始追蹤。</Empty></Glass>}</div>; }
function CardTile({ card, edit, toggle, remove, updateDate }) { const style = BANK_STYLES[card.bank] || BANK_STYLES.其他銀行; const dateRef = useRef(null); return <div className="credit-card" style={{ '--card-a': style[0], '--card-b': style[1], '--card-accent': style[2] }}><div className="card-shine" /><div className="card-top"><span>{card.bank}</span><Landmark size={20} /></div><Chip /><p className="card-number">•••• &nbsp;•••• &nbsp;•••• &nbsp;{String(card.id).slice(-4)}</p><div className="card-bottom"><div><small>CARD HOLDER</small><strong>{card.name}</strong></div><div className="card-due"><small>應還金額</small><strong>{money(card.amount)}</strong></div></div><div className="card-actions"><span className={card.isPaid ? 'paid' : ''}>{card.isPaid ? <><CheckCircle2 size={13} />已結清</> : <><AlertCircle size={13} />待還款</>}</span><div><button title="修改到期日" onClick={() => dateRef.current?.showPicker?.()}><input ref={dateRef} className="card-date" type="date" value={card.dueDate || ''} onChange={(event) => updateDate(card.id, event.target.value)} /></button><button title="編輯" onClick={() => edit(card)}><Edit3 size={14} /></button><button title="切換還款狀態" onClick={() => toggle(card)}><Check size={14} /></button><button title="刪除" onClick={() => remove(card.id)}><Trash2 size={14} /></button></div></div></div>; }
